	...
	for (i = 0; i < nr_pack; i++)
	{
		...
		if (pack->length)
		{
			...
			else
			{
				...
				if (ret != pack->length)
				{
					printf("stream write ret(%d) != pack[%d].length(%d) error:%s\n", ret, i, pack->length, strerror(errno));
					return -1;
				}
			}
		}
